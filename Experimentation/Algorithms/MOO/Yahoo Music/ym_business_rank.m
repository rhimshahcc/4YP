% A function to create a business ranking user row. 

function business_ranking = ym_business_rank(ratings_matrix,row_number)

user_row = ratings_matrix(row_number,:); % extract the row
 
row_length = size(user_row,2); % length of the row (i.e. number of items)

business_values = rand(1,row_length) * 5; % row vector of the time listened values for each item
